...
export const getMovieData = ( movies, handleData, handleLoading, handleError, getNumberofPages, pagenum, first, last ) => {
    ...
    let api = pagenum === undefined ? 
        `http://www.omdbapi.com/?i=tt3896198&apikey=3dc5850a&type=movie&plot=full&s=${dmovies}`
        :
        `http://www.omdbapi.com/?i=tt3896198&apikey=3dc5850a&type=movie&plot=full&s=${dmovies}&page=${pagenum}`
    handleLoading( true )
    API( api )
    .then( res => {
        handleLoading( false )
